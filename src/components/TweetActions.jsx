...
export default function TweetActions({ tweet }) {
  ...
  return (
    <div className="flex justify-start space-x-8 mt-3">
      ...
      <button 
        className={`flex items-center space-x-2 transition-colors ${retweets > 0 ? 'text-green-500' : 'text-gray-500 hover:text-green-500'}`}
      ...
      <button 
        className={`flex items-center space-x-2 transition-colors ${likes > 0 ? 'text-pink-500' : 'text-gray-500 hover:text-pink-500'}`}
        onClick={handleLike}
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
